...
if ($minikubeStatus -ne "Running") {
    ...
    try {
        minikube start --docker-opt="dns=8.8.8.8" --cpus=4 --memory=8192 --driver=docker
        if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Tente iniciar manually com: minikube start --dns-ip=8.8.8.8 --cpus=4 --memory=8192 --driver=docker"
        exit 1
    }
}
else {
...
if (-not (Build-DockerImage -ServiceName "Professor Service" -FolderPath ".\professor-service" -ImageTag "distrischool-professor-tecadm-service:latest" -Emoji "📚")) {
    $buildSuccess = $false
}
...
if (-not (Build-DockerImage -ServiceName "Grades Service" -FolderPath ".\grades-service" -ImageTag "distrischool-grades-service:latest" -Emoji "📝")) {
    $buildSuccess = $false
}

if (-not (Build-DockerImage -ServiceName "Communication Service" -FolderPath ".\communication-service" -ImageTag "distrischool-communication-service:latest" -Emoji "📬")) {
    $buildSuccess = $false
}
...
Write-Info "🔐 Aplicando configurações de autenticação (JWT)..."
try {
    kubectl apply -f k8s-manifests/auth-secret.yaml
    if ($LASTEXITCODE -ne 0) {
        throw "Falha ao aplicar Secret de autenticação"
    }
    kubectl apply -f k8s-manifests/auth-configmap.yaml
    if ($LASTEXITCODE -ne 0) {
        throw "Falha ao aplicar ConfigMap de autenticação"
    }
    Write-Success "✅ Configurações de autenticação aplicadas."
}
catch {
    Write-Error "❌ Erro ao aplicar configurações de autenticação: $_"
    exit 1
}
...
Write-Info ""
Write-Info "🔍 Aplicando Stack de Monitoramento (Prometheus + Grafana)..."
try {
    kubectl apply -f k8s-manifests/monitoring/
    if ($LASTEXITCODE -ne 0) {
        throw "Falha ao aplicar manifests de monitoramento"
    }
    Write-Info "   Reiniciando Prometheus e Grafana para aplicar novas configurações..."
    kubectl rollout restart deployment/prometheus
    kubectl rollout restart deployment/grafana
    Write-Success "✅ Stack de monitoramento aplicada."
}
catch {
    Write-Error "❌ Erro ao aplicar stack de monitoramento: $_"
    exit 1
}
...
Write-Info "   Aguardando Prometheus..."
kubectl wait --for=condition=ready pod -l app=prometheus --timeout=120s 2>&1 | Out-Null
Write-Info "   Aguardando Grafana..."
kubectl wait --for=condition=ready pod -l app=grafana --timeout=120s 2>&1 | Out-Null
...
Write-Warning "   -> Forçando reinício (rollout) do 'professor-tecadm-deployment'..."
kubectl rollout restart deployment/professor-tecadm-deployment # <-- ADICIONADO

Write-Info ""
Write-Info "👨‍🎓 Aplicando Aluno Service..."
kubectl apply -f k8s-manifests/aluno-service/
Write-Warning "   -> Forçando reinício (rollout) do 'aluno-deployment'..."
kubectl rollout restart deployment/aluno-deployment # <-- ADICIONADO

Write-Info ""
Write-Info "👤 Aplicando User Service..."
kubectl apply -f k8s-manifests/user-service/
Write-Warning "   -> Forçando reinício (rollout) do 'user-deployment'..."
kubectl rollout restart deployment/user-deployment # <-- ADICIONADO

Write-Info ""
Write-Info "🌐 Aplicando API Gateway..."
kubectl apply -f k8s-manifests/api-gateway/
Write-Warning "   -> Forçando reinício (rollout) do 'api-gateway-deployment'..."
kubectl rollout restart deployment/api-gateway-deployment # <-- ADICIONADO

Write-Info ""
Write-Info "📝 Aplicando Grades Service..."
kubectl apply -f k8s-manifests/grades-service/
Write-Warning "   -> Forçando reinício (rollout) do 'grades-deployment'..."
kubectl rollout restart deployment/grades-deployment

Write-Info ""
Write-Info "📬 Aplicando Communication Service..."
kubectl apply -f k8s-manifests/communication-service/
Write-Warning "   -> Forçando reinício (rollout) do 'communication-deployment'..."
kubectl rollout restart deployment/communication-deployment
...
Wait-ForDeploymentReady -DeploymentName "grades-deployment"
Wait-ForDeploymentReady -DeploymentName "communication-deployment"
...
Write-Warning "   -> Forçando reinício (rollout) do 'frontend-deployment'..."
kubectl rollout restart deployment/frontend-deployment # <-- ADICIONADO
...
if ($hostsContent -match "distrischool.local") {
    ...
    $hostsContent | Where-Object { $_ -notmatch "distrischool.local" } | Set-Content $tempFile
...
# Use StreamWriter to avoid file locking issues
...
Write-Host "   Prometheus: http://$(minikube ip):30090" -ForegroundColor Cyan
Write-Host "   Grafana:    http://$(minikube ip):30030 (admin/admin)" -ForegroundColor Cyan
Write-Info ""
...
Write-Info "3. Acessar Prometheus:"
Write-Host "   minikube service prometheus-service --url" -ForegroundColor Yellow
Write-Info ""

Write-Info "4. Acessar Grafana:"
Write-Host "   minikube service grafana-service --url" -ForegroundColor Yellow
Write-Info "   (Usuário: admin, Senha: admin)"
Write-Info ""

Write-Info "5. Acessar RabbitMQ Management:"
